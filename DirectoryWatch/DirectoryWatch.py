...
class DirectoryWatch:
    ...
    def watch_stop(self, action, full_filename):
        ...
        try:
            # unpack timestamp from last read
            last_action, timestamp = self.watch_list[full_filename]
            # If the file is not updating after 30 seconds, finished run.
            # Force that the file must have gone through an update before, not just created.
            # This is to ensure that the file has been used by the ALD software.
            if (dt.datetime.now() - timestamp).total_seconds() > 30 and last_action == 3:
            ...
            elif last_action == 3:  # if file is updated
                self.watch_list[full_filename] = action, dt.datetime.now()  # update check time
            elif last_action != 3:  # if file has other status
